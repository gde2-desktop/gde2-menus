...
def print_directory(dir, parent_path = None):
    ...
    for item in dir.get_contents():
        type = item.get_type()
        if type == gde2menu.TYPE_ENTRY:
            print_entry(item, path)
        elif type == gde2menu.TYPE_DIRECTORY:
            print_directory(item, path)
        elif type == gde2menu.TYPE_ALIAS:
            aliased = item.get_item()
            if aliased.get_type() == gde2menu.TYPE_ENTRY:
                print_entry(aliased, path)
        elif type in [ gde2menu.TYPE_HEADER, gde2menu.TYPE_SEPARATOR ]:
            pass
        else:
...
def main(args):
    ...
    if options.file:
        ...
        menu_file = 'gde2-applications.menu'

    flags = gde2menu.FLAGS_NONE
    if options.exclude:
        flags |= gde2menu.FLAGS_INCLUDE_EXCLUDED
    if options.nodisplay:
        flags |= gde2menu.FLAGS_INCLUDE_NODISPLAY

    tree = gde2menu.lookup_tree(menu_file, flags)
